...
- Custom Rust SOOD/MOO protocol implementation needs its own wire-protocol study from the official reference SDK (`node-roon-api`) and community ports — no existing crate is being relied on as a dependency (see TECH_STACK.md's "Explicitly rejected"). This is documenting the transport the officially-supported API modules already use, not reverse-engineering Roon's internal/unofficial protocol surface (see NORTH-STAR.md's non-goals). "Not relying on it" is about not taking the crate as a dependency, not about the code itself: two MIT-licensed (one dual MIT/Apache-2.0) community Rust ports exist as legally-clear reference material to read from and, if useful, copy/adapt snippets from (with their copyright notice retained) — [`shin1ohno/roon-rs`](https://github.com/shin1ohno/roon-rs) (published as the `roon-api` crate, MIT OR Apache-2.0) and [`TheAppgineer/rust-roon-api`](https://github.com/TheAppgineer/rust-roon-api) (git-only, not on crates.io, MIT, alpha stage).
