...
- Renamed `release.yml` → `publish.yml` (2026-09-07): GitHub had a stuck workflow registration under the `release.yml` path — it kept running `ci.yml`'s `check`/`version-bump-check` jobs on every plain push despite the file's `on:` block always having been tag-only, and survived a delete-then-recreate under the same filename. Renaming the file broke the stale mapping. No trigger/behavior change intended — still tag-only (`v*.*.*`), still never runs on a plain push.
